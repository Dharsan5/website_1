...
export default function Hero() {
  return (
    <section className="relative h-[calc(100vh-4rem)] md:h-[calc(100vh-5rem)] rounded-[2rem] overflow-hidden bg-noise">
      <video autoPlay className="absolute inset-0 w-full h-full object-cover opacity-70 video-zoom" loop muted playsInline aria-hidden>
      ...
      <div className="absolute inset-0 flex flex-col justify-end p-8 md:p-12 z-20">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-end">
          <div className="md:col-span-8">
            <h1 className="font-display-lg-mobile md:font-display-lg text-primary text-[12vw] md:text-[16vw] leading-none tracking-tighter fade-up">
              Prisma<sup className="text-[4.5vw] hero-title-sup">*</sup>
            </h1>
          </div>
          <div className="md:col-span-4 flex flex-col items-start md:items-end text-left md:text-right gap-6">
            <p className="font-body-lg text-primary/70 max-w-sm fade-up" style={{ animationDelay: '120ms' }}>
              Prisma is a worldwide network of visual artists exploring the intersection of light, shadow, and narrative.
            </p>
            <button className="flex items-center gap-4 bg-primary-container text-on-primary-container rounded-full pl-6 pr-2 py-2 group hover:bg-primary transition-colors cta-glow" aria-label="Join the lab">
      ...
      <div className="absolute inset-0 hero-overlay pointer-events-none z-10"></div>
    </section>
  )
}
